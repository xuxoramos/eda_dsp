...
You can also look into the complete ds program table mapped with the categories we created earlier. This table is in `dsp.csv`. Using the `TYPE`, `DELIVERY`, `DEPT_CAT` and `PGRM_CAT` columns to look for programs in the table is much easier that using the raw columns in the original dataset.
